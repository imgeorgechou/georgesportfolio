  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
        scrolled ? 'backdrop-blur-md bg-paper/80 border-b border-rule' : 'bg-transparent'
      }`}
    >
      <div className="mx-auto max-w-[1400px] px-6 lg:px-12 h-16 flex items-center justify-between">
        <a href="#home" className="flex items-center gap-2 group cursor-pointer">
          <span className="grid grid-cols-2 gap-[3px]">
            {dotColors.map((c, i) => (
              <span key={i} className="w-[7px] h-[7px] rounded-full transition-transform duration-300 group-hover:scale-125" style={{ background: c }} />
            ))}
          </span>
          <span className="font-display text-[17px] font-semibold tracking-tight">周致祥</span>
        ...
        <nav className="hidden md:flex items-center gap-1">
          {items.map((it, i) => (
            ...
            >
              ...
              <span className={`transition-colors duration-300 ${active === it.id ? 'text-ink' : 'text-ink-soft group-hover:text-ink'}`}>
                {it.label}
              </span>
              {active === it.id && (
                <motion.span
